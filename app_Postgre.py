...
def login():
    if request.method == "POST":
        ...
        entitas = request.form.get("entitas")

        if username == "admin" and password == "13579":
            session["user"] = "admin"
            session["entitas"] = entitas
...
def index():
    ...
    if session["user"] == "admin":
        ...
        return redirect(url_for("index_viewrekap"))
...
def index_viewrekap():
    ...
    # Jenis anggota kedua (tambahan tetap) adalah AkunInternal
    ...
    if session["user"] == "admin":
        if request.method == "POST":
            ...
            if anggota_filter:
                # Tambahkan filter KodeAnggota juga
